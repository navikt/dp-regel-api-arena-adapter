...
internal class FuelHttpClient(val baseUrl: String, private val apiKey: String? = null) {
    ...
    inline fun <reified T : Any> get(
        path: String,
        configure: (Request) -> Unit = {},
    ): ResponseResultOf<T> = run(Method.GET, path, configure)

    inline fun <reified T : Any> post(
        path: String,
        configure: (Request) -> Unit,
    ): ResponseResultOf<T> = run(Method.POST, path, configure)

    inline fun <reified T : Any> run(
        method: Method,
        path: String,
        configure: (Request) -> Unit,
    ): ResponseResultOf<T> = request(method, path, configure).run { this.responseObject() }
}
